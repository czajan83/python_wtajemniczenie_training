...
def cinema_movies_schedule():
    ...
    cinema_hour_input = input("At what time the earliest? (HH:MM): ")
    cinema_hour = time.fromisoformat(cinema_hour_input)
    weekday_number = cinema_date.isoweekday()
    weekday = Weekday(weekday_number)
    operating_hours = generate_random_times()
    print(operating_hours)
    print("This day you can watch:")
    for index, movie in enumerate(cinema_schedule.get_movies_by_weekday(weekday)):
        if cinema_hour < operating_hours[index]:
            print(operating_hours[index], movie)
...
def generate_random_times():
    ...
    for _ in range(1, 3):
        ...
        random_times.append(time(hour=hour, minute=minute))
