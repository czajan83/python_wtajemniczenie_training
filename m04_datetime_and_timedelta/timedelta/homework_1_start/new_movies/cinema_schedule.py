...
def generate_february_week_schedule(schedule, provided_date=None):
    if provided_date is None:
        provided_date = date(year=2021, month=2, day=21)

    print(provided_date.isoweekday())

    result = {}
    for weekday, showtimes in schedule.items():
        particular_weekday_in_february = provided_date.replace(
            day=provided_date.day - provided_date.isoweekday() + weekday.value)
        result[weekday] = [
